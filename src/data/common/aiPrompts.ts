...
5. Shared Passages / Contexts: If multiple questions refer to the SAME reading passage, experiment description, or image, you MUST duplicate that exact stimulus object across ALL associated questions. For long text passages, use a stimulus of type "text" and use <br><br> for paragraph breaks. Example: "stimulus": { "type": "text", "data": "Paragraph 1...<br><br>Paragraph 2..." }
7. FRQ Rules: If you encounter a Free Response Question, treat the main scenario/graph as the "stimulus". Treat each sub-part (Part A, Part B, Part C) as entries in a "parts" array. Set "questionType" to "frq" instead of providing "options". Each part can also have its own "stimulus" or "type" field if it contains a diagram. Label long or short FRQs using the "partLabel" field (e.g., "Long FRQ - Part A", "Short FRQ - Part A", or leave it if unsure).
...
7. FRQ Rules: If you encounter a Free Response Question, treat the main scenario/table/graph as the "stimulus". Treat each sub-part as entries in a "parts" array. Set "questionType" to "frq". Use partLabel values like "A", "B", "C" for main parts. If a part has sub-parts (i, ii, iii), include them as separate entries with partLabel "A.i", "A.ii", etc. Each part can also have its own "stimulus" or "type" field if it contains a graph or grid. Label long or short FRQs using the "partLabel" field (e.g., "Long FRQ - Part A", "Short FRQ - Part A", or leave it if unsure).
